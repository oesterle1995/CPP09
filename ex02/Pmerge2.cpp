...
/*   Updated: 2026/09/02 20:09:53 by aoesterl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::vector<t_element> Pmerge::generate_jacob_tab(size_t size)
{
...
{ 
    std::vector<t_element> order_b = generate_jacob_tab(pending.size()); // 1 3 2 5 4  => 0 2 1 4 3
    size_t high;
    main_chain.insert(main_chain.begin(), pending[0]); 
    for(size_t i = 1; i < order_b.size(); i++)
    {
        high = order_b[i].value + i;
